...
//! CoreT (problem.c:181/230/243/357): the full-field copy and the two
...
pub fn copyFull(comptime CoreT: type, self: *CoreT, dst: *CoreT.FieldT, src: *const CoreT.FieldT) void {
...
fn Ctx(comptime CoreT: type) type {
    return struct {
        sim: *CoreT,
        dst: *CoreT.FieldT,
        a_f: *const CoreT.FieldT,
        b_f: *const CoreT.FieldT,
        c_f: *const CoreT.FieldT = undefined,
        f1: f64,
        f2: f64,
    };
}
...
pub fn deriv(comptime CoreT: type, self: *CoreT, dst: *CoreT.FieldT, a_f: *const CoreT.FieldT, b_f: *const CoreT.FieldT, delta: f64) void {
    var ctx = Ctx(CoreT){ .sim = self, .dst = dst, .a_f = a_f, .b_f = b_f, .f1 = 1.0 / delta, .f2 = -1.0 / delta };
    _ = threading.parallelRange(Ctx(CoreT), &ctx, self.team, 0, self.nyi(), derivWorker(CoreT));
}

fn derivWorker(comptime CoreT: type) fn (*Ctx(CoreT), i64, i64, *threading.ChunkResult) void {
    return struct {
...
pub fn combine(comptime CoreT: type, self: *CoreT, dst: *CoreT.FieldT, a_f: *const CoreT.FieldT, f1: f64, b_f: *const CoreT.FieldT, f2: f64, c_f: *const CoreT.FieldT) void {
    var ctx = Ctx(CoreT){ .sim = self, .dst = dst, .a_f = a_f, .b_f = b_f, .c_f = c_f, .f1 = f1, .f2 = f2 };
    _ = threading.parallelRange(Ctx(CoreT), &ctx, self.team, 0, self.nyi(), combineWorker(CoreT));
}

fn combineWorker(comptime CoreT: type) fn (*Ctx(CoreT), i64, i64, *threading.ChunkResult) void {
    return struct {
        fn w(ctx: *Ctx(CoreT), iy0: i64, iy1: i64, res: *threading.ChunkResult) void {
            _ = res;
            const NV = CoreT.nv;
