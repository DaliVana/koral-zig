...
//! update_entropy), generic over CoreT. `Sim.updateEntropy` owns the timer
...
pub fn copyEntropyCount(comptime CoreT: type, self: *CoreT) void {
    var iz: i64 = 0;
...
pub fn rowsFn(comptime CoreT: type) fn (*CoreT, i64, i64) Error!void {
    return struct {
        fn w(s: *CoreT, iy0: i64, iy1: i64) Error!void {
            return entropyRows(CoreT, s, iy0, iy1);
        }
    }.w;
}
...
fn entropyRows(comptime CoreT: type, self: *CoreT, iy0: i64, iy1: i64) Error!void {
    const cfg = CoreT.Cfg;
    ...
    while (iz < self.nzi()) : (iz += 1) {
        ...
        while (iy < iy1) : (iy += 1) {
            ...
            while (ix < self.nxi()) : (ix += 1) {
                ...
                pp[L.index(.entr)] = hydro.sFromU(pp[L.index(.rho)], pp[L.index(.uu)], self.phys.gam);
                ...
                try p2u_mod.p2uMhd(cfg, pp, &uu, &geom, self.phys.gam);
