...
test "conv_vels: u·u = -1 after VELR→VEL4 and VEL3→VEL4 everywhere" {
    ...
    for (test_geoms) |tg| {
        ...
        for (0..20) |_| {
            ...
            const uc4 = try relele.convVels(.{ 0, v[0], v[1], v[2] }, .velr, .vel4, &geo.gg, &geo.GG);
            const uc4cov = relele.indices21(uc4, &geo.gg);
            try expectClose(relele.dot(uc4, uc4cov), -1.0, 1e-12);

            // through VEL3 and back up to VEL4
            const uc3 = try relele.convVels(uc4, .vel4, .vel3, &geo.gg, &geo.GG);
            const uc4b = try relele.convVels(uc3, .vel3, .vel4, &geo.gg, &geo.GG);
            const uc4bcov = relele.indices21(uc4b, &geo.gg);
...
test "conv_vels: all round trips return the input" {
    ...
    for (test_geoms) |tg| {
        ...
        for (0..10) |_| {
            ...
            const via4 = try relele.convVels(try relele.convVels(vr, .velr, .vel4, &geo.gg, &geo.GG), .vel4, .velr, &geo.gg, &geo.GG);
            for (1..4) |i| try expectClose(via4[i], vr[i], 1e-13);

            const via3 = try relele.convVels(try relele.convVels(vr, .velr, .vel3, &geo.gg, &geo.GG), .vel3, .velr, &geo.gg, &geo.GG);
            for (1..4) |i| try expectClose(via3[i], vr[i], 1e-13);

            const uc4 = try relele.convVels(vr, .velr, .vel4, &geo.gg, &geo.GG);
            const back4 = try relele.convVels(try relele.convVels(uc4, .vel4, .vel3, &geo.gg, &geo.GG), .vel3, .vel4, &geo.gg, &geo.GG);
...
test "conv_vels: superluminal VEL3 is rejected" {
    ...
    try std.testing.expectError(
        ...
        relele.convVels(bad, .vel3, .vel4, &geo.gg, &geo.GG),
    );
}
...
test "normal observer: n·n = -1 and its VELR vanishes" {
    for (test_geoms) |tg| {
        const geo = geometryAt(tg.coords, tg.mp, tg.x);
        const ncon = relele.normalObs4vel(&geo.GG);
        const ncov = relele.indices21(ncon, &geo.gg);
        try expectClose(relele.dot(ncon, ncov), -1.0, 1e-13);

        const nrel = relele.normalObsRelvel(&geo.GG);
...
test "indices: raise∘lower is the identity" {
    ...
    for (test_geoms) |tg| {
        ...
        const back = relele.indices12(relele.indices21(a, &geo.gg), &geo.GG);
        ...
        const tback = relele.indices1122(relele.indices2211(t, &geo.gg), &geo.GG);
        for (0..4) |i| {
...
test "Lorentz matrix preserves the coordinate metric: g(Lx,Ly) = g(x,y)" {
    ...
    for (test_geoms) |tg| {
        ...
        const l = try frames.lorentzLab2Ff(v, &geo.gg, &geo.GG);
...
test "Lorentz lab2ff maps the fluid velocity to α·(normal observer)" {
    for (test_geoms) |tg| {
        ...
        const l = try frames.lorentzLab2Ff(v, &geo.gg, &geo.GG);
        const u = try relele.convVels(.{ 0, v[0], v[1], v[2] }, .velr, .vel4, &geo.gg, &geo.GG);
        const lu = relele.multiply2(u, l);
        const w = relele.normalObs4vel(&geo.GG);
...
test "Lorentz lab2ff and ff2lab matrices are mutual inverses" {
    for (test_geoms) |tg| {
        ...
        const l1 = try frames.lorentzLab2Ff(v, &geo.gg, &geo.GG);
        const l2 = try frames.lorentzFf2Lab(v, &geo.gg, &geo.GG);
        for (0..4) |i| {
            for (0..4) |j| {
                var s: f64 = 0;
...
test "vector boosts round-trip exactly (α cancels between directions)" {
    for (test_geoms) |tg| {
        ...
        const there = try frames.boost2Lab2Ff(a, v, &geo.gg, &geo.GG);
        const back = try frames.boost2Ff2Lab(there, v, &geo.gg, &geo.GG);
        for (0..4) |i| try expectClose(back[i], a[i], 1e-12);
    }
}
...
test "tensor boost round-trip pins the C α-quirk (ff2lab applies no α)" {
    ...
    for (test_geoms) |tg| {
        ...
        var there = try frames.boost22Lab2Ff(t, v, &geo.gg, &geo.GG);
        for (0..4) |i| {
            there[i][0] /= geo.alpha;
            there[0][i] /= geo.alpha;
        }
        const back = try frames.boost22Ff2Lab(there, v, &geo.gg, &geo.GG);
        for (0..4) |i| {
            for (0..4) |j| {
...
test "trans2_coco preserves u·u between coordinate systems" {
    ...
    for (pairs) |pc| {
        ...
        const u = try relele.convVels(.{ 0, v[0], v[1], v[2] }, .velr, .vel4, &g1.gg, &g1.GG);
        const uc2 = frames.trans2Coco(pc.x, u, pc.from, pc.to, pc.mp);

        const s1 = relele.dot(u, relele.indices21(u, &g1.gg));
        const s2 = relele.dot(uc2, relele.indices21(uc2, &g2.gg));
...
test "b·u = 0 and bsq = b·b for the magnetic four-vector" {
    ...
    for (test_geoms) |tg| {
        ...
        const u = try relele.convVelsBoth(.{ 0, v[0], v[1], v[2] }, .velr, &geo.gg, &geo.GG);
        const B = [3]f64{ 0.3, -0.7, 0.2 };
        const b = mhd.bconBcovBsqFrom4vel(B, u.con, u.cov, &geo.gg);
...
test "p2u momentum rows equal gdet·T^t_i from calc_Tij" {
    ...
    for (test_geoms) |tg| {
        ...
        const tmix = relele.indices2221(t, &geo.gg); // T^μ_ν
        for (1..4) |i| {
            try expectClose(uu[L.index(.vx) + i - 1], geo.gdet * tmix[0][i], 1e-12);
        }
        // energy row: uu[UU] = gdet·(T^t_t + rho·u^t), assembled via utp1
        const u = try relele.convVelsBoth(.{ 0, pp[2], pp[3], pp[4] }, .velr, &geo.gg, &geo.GG);
