...
**Fixed (2026-07-07):** Added an optional per-cell BL sidecar to `MetricCache`, built once at init only when `dynamo`/`radviscosity` is set (`bl_cache` InitOpt; `Sim.init` also points `out_coords` at `.bl` so the already-allocated-but-unused `dxdx_my2out` store becomes the MYCOORDS→BL Jacobian — no new Jacobian array). New accessors: `blGeom(ix,iy,iz)` (a `*const Geometry`, filled by `fillBl` = the exact `geometryBLat` chain; its `.xxvec` carries BL {r,θ}), `jacMy2Bl(ix,iy,iz)`, and `gdet(ix,iy,iz)` (`self.g[ci*20+19]`). Callers rewired: (1)+(2) `dynamo.fieldAngle` now takes the cached MKS2/BL geometries + Jacobian and calls a new `frames.transPmhdCocoJ` (a precomputed-Jacobian variant sharing `transPmhdCoco`'s body — the old code recomputed the same `coco.dxdx` twice per cell; it is now computed zero times per cell); the per-cell `geometryAt(.bl)` is gone. (3) The redundant `coco.cocoN` reads in `scaleHeightCols`, `deltaARows`, and `radvisc.calcRadViscCoeff` are replaced by reading BL {r,θ} from the cached `blGeom(...).xxvec`; `scaleHeightCols` also drops its `fillGeometry` in favour of `gdet(...)`. (4) `ct.calcBfromACore` and `calcDivB`'s `pB` helper read `cache.gdet(...)` instead of building a full `Geometry`. The dead `geomBLat` wrapper and now-unused `coco`/`precompute` imports were removed. **Bit-identity:** a new gate (dynamo_tests.zig) asserts `blGeom == geometryBLat`, `jacMy2Bl == coco.dxdx`, and `gdet == fillGeometry().gdet` at the u64 level over the full 48×44 grid + ghost frame (2700 cells, 0 mismatches); `transPmhdCocoJ` is arithmetically identical to `transPmhdCoco` (same Jacobian, same `multiply2`), and every downstream value fed to the physics is unchanged — so the dynamo/visc/puffystep C-goldens are byte-for-byte unaffected.
...
**Fixed (2026-07-07):** Implemented exactly as suggested. `MetricCache.krBlock` added; `cellIndex`/`faceIndex`/`kr` marked `inline`. metricSource (sim.zig:824) and calcShearLab (radvisc.zig:176) now fetch `kr_blk` once per cell and index the flat block (`kr_blk[l*16+nu*4+k]`, `[k*16+i*4+j]`, `[i*16+i*4+k]`); the two metric/tests.zig call sites keep `kr()`. Bit-identity: the block accessor reads the exact same `kris[...]` slots kr() did, so every fetched Γ is the same bits. Verified — default suite (incl. `calc_Tij vs C`, `f_flux_prime vs C`) exit 0, full `-Dslow-tests` exit 0, and the chaotic PUFFY pipeline/dynamo/radvisc goldens are line-for-line identical to the pre-change baseline (empty diff).
...
**Fixed (2026-07-07):** Split `calcTij` into a thin wrapper + `calcTijFromState(cfg, pp, geom, gamma, u: relele.ConCov, bcon, bsq)` holding just the tensor assembly. `fFluxPrime` now solves `u = convVelsBoth(vcon, .velr, …)` and `bconBcovBsqFrom4vel(…, u.con, u.cov, …)` once at the top and passes them into `calcTijFromState`, deleting the duplicate `uconUcovFromPrims` + `bconBcovBsqFrom4vel` that `calcTij` used to run internally. The five other `calcTij` callers are unchanged. Bit-identity: fFluxPrime's `u`/`bcon`/`bsq` are the *same pure functions on the same inputs* calcTij evaluated, and `calcTijFromState`'s body is the byte-identical arithmetic of the old calcTij tail — so T is the same bits. Verified — `calc_Tij vs C` and `f_flux_prime vs C` goldens pass in the default suite, `-Dslow-tests` exit 0, and the chaotic PUFFY pipeline is line-for-line identical to baseline. (The optional checkFloorsMhd/calcUtp1 threading was left for a later pass — the floor-fired invalidation adds risk without pinning goldens.)
...
**Fixed (2026-07-07):** Implemented as sub-parts (1)+(2)+(3). Each generic split into a `comptime full: bool` core with the existing name = full wrapper (every caller / golden / opacity test untouched) and a new `…Slim` wrapper: `radforce.fillRadStateSlim(G)` skips `sgas` (only `state00`.sgas is read — the entropy rung), `radforce.calcGiFromStateSlim(G)` skips the lab→ff boost entirely (residual/f1dErr read only `gi.ff[0]`, supplied bit-identically by the direct fluid-frame expression, plus the boost-independent `gi.lab`), and `opacities.calcOpacitiesFromStateG(…, full)` / `calcKappaFromStateG` gate the `rad_num`/`gas_ross`/`gas_ffross`/`tot_emissivity` channels. Wired via a new `ImplicitParams.slim_state=true` that toggles solve4dPrim's per-iteration fills (base/perturb/damping/V4-batch); `state00` stays full; residualG + f1dErr are always slim. Sub-part (4) (reuse `st.ucon/ucov`+`geom.alpha` *inside* the boost) is moot on the slim path (no boost at all) and left as-is on the full path. Verified bit-identical two ways: new simd_tests gates (`slim == full` bitwise on every consumed field incl. grey params; `slim_state` on == off bitwise over the full 6-rung ladder, 213/300 converged, 101 deep-rung) and a git-stash A/B showing the full path and all end-to-end goldens byte-for-byte unchanged (opac golden opacities 2.420e-14 / kappa-chi 2.548e-15 / Gi 1.719e-14; implicit golden ok-agree 327/336, rung 278/286, worst pp dev 5.868e-8 rec 161; golden_puffystep 384×360 all 4 steps identical, flags 100%). Default suite (ReleaseSafe) exit 0. NOT done here: the analogous slim path for `calcChi` (that is the separate finding below, `radforce.zig:320`), so the wavespeed τ-limiter and radvisc still run the full fill.
...
**Fixed (2026-07-07):** Implemented exactly as the fix describes. The `comptime full: bool` gate on `calcOpacitiesFromStateG` / `calcKappaFromStateG` became a 3-level `comptime OpacLevel` enum: `.full` (every channel — golden/opacity tests, calc_kappa, tot_emissivity), `.residual` (gas_abs + rad_abs + rad_ross — the implicit residual, unchanged from the #4 `full == false`), and the new `.chi` (gas_abs only). One formula body serves all three, so every *consumed* channel keeps its exact expression and the narrower levels are bit-identical to `.full` on the fields they populate. New `radforce.calcChiSlim` (kept alongside `calcChi`, the C-parity golden entry point) computes tempsFromUrho once, ucon/ucov + b² exactly as fillRadStateCoreG, then κ = gas_abs via the `.chi` level plus the standalone κ_es(Te) — skipping calcRij, the Ehat contraction, lteTfromE, sgas, the state-path kappaes, and the four Trad-dependent channels. The two production callers (sim.zig wavespeed τ-limiter, radvisc.zig mfp) now call `calcChiSlim`; the two golden tests stay on `calcChi`. Verified bit-identical: a new simd_tests gate (`calcChiSlim == calcChi` bitwise over 200 fuzz cases × PUFFY+grey × mink+ks × B on/off), the calc_kappa/kappaes/chi golden unchanged at 2.548e-15, and the full 64×60 forced-dt PUFFY pipeline golden (which drives calcChiSlim through both wavespeeds and radvisc) byte-for-byte identical to the pre-change build on all 4 steps (1.006e-8 / 1.115e-3 / 1.089e-2 / 8.601e-2, flags 100%). This closes the last deferred sub-part of the #4 finding (`calcChi` was explicitly left full there).
...
**Fixed (2026-07-07):** Implemented exactly as described, taking the "thin wrapper" option. `solve4dPrim`'s Newton loop moved to a new `solve4dPrimCore(uu00, state00: *const RadState, pp0in: *const [NV]f64, …)`; `solve4dPrim` kept its original signature as a wrapper that fills state00 (the *full* fill — the entropy rungs read state00.sgas) and runs the bisect, then delegates — so the direct implicit_tests.zig call sites are untouched. `solveImplicitLab` now computes state00 + pp0 once before the rung table and calls `solve4dPrimCore` for all six rungs, with an early `return failres` when the state00 fill fails (output-identical: the old code returned failres after all six rungs failed the same fill). For a rung-0-failing cell the six rungs now share one state00 fill + one bisection instead of recomputing them 6× — the bisect alone is up to 50 f1dErr evaluations (each a fillRadStateSlim + calcGiFromStateSlim). Verified bit-identical: a new simd_tests gate asserts `solve4dPrim` (recomputes per rung — the pre-hoist path) equals `solve4dPrimCore` (fed the once-hoisted pair) bitwise on ok/iters/uu/pp across all six rungs × 200 fuzz cases (1200 checks), and the implicit golden (327/336 ok-agree, 278/286 rung, worst 5.868e-8 rec 161) plus the 64×60 PUFFY pipeline golden (all 4 steps) are byte-for-byte identical to the pre-change build.
...
**Fixed (2026-07-07):** Build default left untouched (per the fix note). PROBLEMS/puffy/main.zig now prints `build={s}` (`@tagName(@import("builtin").mode)`) in its startup line, plus a prominent `*** WARNING: Debug build … Rebuild with -Doptimize=ReleaseFast ***` when `builtin.mode == .Debug`.
